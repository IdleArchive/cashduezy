...
export async function POST(req: Request) {
  try {
    ...
    const stripe = new Stripe(secretKey); // <-- apiVersion omitted (or pin to "2024-06-20")
    ...
    if (body?.user_id) {
  sessionConfig.client_reference_id = body.user_id;
  sessionConfig.metadata = { ...(sessionConfig.metadata ?? {}), user_id: body.user_id };
}
